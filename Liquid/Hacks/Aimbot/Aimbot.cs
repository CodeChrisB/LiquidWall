    ...
    static class Aimbot
    {
                ...
                if (Globals.TriggerPressOnlyEnabled && !Globals.AimShootOnCollide)
                {
                    if ((GetAsyncKeyState(Globals.TriggerKey) & 0x8000) > 0)
                    {
                        if (CBasePlayer.CrosshairID > 0 && CBasePlayer.CrosshairID < EngineDLL.MaxPlayer + 2)
                        {
                            ...
                            {
                                Thread.Sleep(Globals.TriggerDelayMS);
        ...
        public static void AimAssistThread()
        {
            while (true)
            {
                ...
                if (closestEntityPos.x != 200 && (GetAsyncKeyState(Globals.TriggerKey) & 0x8000) > 0)
                {
                    double distance =Math.Sqrt(closestEntityPos.x * closestEntityPos.x + closestEntityPos.y * closestEntityPos.y);
                        ...
                        //nice
                        double x = Abs(CBasePlayer.ViewAngle.x - AimAt.x);
                        double y = Abs(CBasePlayer.ViewAngle.y - AimAt.y);
                    double avg = ((x + y) / 2);
                    if (avg<Globals.AimAssistDistance/100)
